...
hub 只监听 `127.0.0.1`，这是故意的。公网访问必须经[反向代理](/install/reverse-proxy/)，由反代把域名转到 hub 端口。确认 hub 在跑：`systemctl status monitor-hub`。
...
ABI v1 已停用，仅剩 v2。已安装的 v1 插件不再加载，需对着 v2 重编并重新上传。见[插件系统总览](/extend/plugins/)的破坏性变更说明。
...
流量按**本地日**切分。Docker 部署不设 `TZ` 会默认 UTC，天边界算错，统计就偏——一定要设 `TZ`（见 [Docker 部署](/install/docker/)）。另外三个流量数字（上行/下行/合计）的口径、重置日与配额算法各不相同，对账时注意用的是哪一个。
